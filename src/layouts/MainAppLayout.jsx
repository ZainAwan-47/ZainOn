...
// Call Components & Providers
import CallProvider from '../providers/CallProvider';
import { MinimizedCallBanner } from '../features/calls/components/GlobalCallManager';
    ...
    const handleMarkAllNotificationsRead = async () => {
        try {
            ...
            unreadMsgNotifs.forEach(n => {
                if (!msgsByConv[n.targetId]) {
                    msgsByConv[n.targetId] = { isGroup: n.type === 'group_message', msgs: [] };
                }
                msgsByConv[n.targetId].msgs.push({ id: n.messageId });
            });

            const markSeenPromises = Object.entries(msgsByConv).map(([convId, data]) => {
                return messageService.markAsSeen(convId, user?.uid, data.msgs, data.isGroup);
            });
            ...
            console.error("Failed to process advanced mark all read:", error);
        }
    };
    ...
    const handleNotificationNavigate = async (notification) => {
        ...
        try {
            ...
            if (messageId && targetId && (type === 'direct_message' || type === 'group_message')) {
                await messageService.markAsSeen(targetId, user?.uid, [{ id: messageId }], type === 'group_message');
            }
        } catch (err) {
        ...
        <CallProvider>
            <div className="fixed inset-0 h-screen w-screen overflow-hidden bg-[var(--bg-main)] text-[var(--text-primary)] font-sans flex select-none transition-colors duration-300">
                ...
                <aside className={`fixed md:relative inset-y-0 left-0 z-50 w-[85%] max-w-[336px] md:w-[336px] md:min-w-[336px] bg-[var(--bg-surface)] border-r border-[var(--border-color)] flex flex-col shrink-0 transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
                    ...
                    <div className="flex-1 overflow-y-auto p-3 space-y-1.5 bg-[var(--bg-surface)] relative z-20">
                        {isSearchActive ? (
                            <UserSearchResults
                                results={searchResults}
                                    ...
                                    {isThemeMenuOpen && (
                                        <div className="absolute bottom-12 right-0 w-44 bg-[var(--bg-surface)] border border-[var(--border-color)] rounded-2xl shadow-2xl p-1.5 z-50 flex flex-col space-y-1 animate-auth-card select-none">
                                            ...
                                            <button
                                                ...
                                                className={`w-full px-3 py-2 rounded-xl text-xs font-semibold flex items-center justify-between transition-colors cursor-pointer ${themeContext?.theme === 'light' ? 'bg-[var(--color-primary)] text-white' : 'text-[var(--text-primary)] hover:bg-[var(--bg-surface-hover)]'}`}
                                            >
                                            ...
                                            <button
                                                ...
                                                className={`w-full px-3 py-2 rounded-xl text-xs font-semibold flex items-center justify-between transition-colors cursor-pointer ${themeContext?.theme === 'dark' ? 'bg-[var(--color-primary)] text-white' : 'text-[var(--text-primary)] hover:bg-[var(--bg-surface-hover)]'}`}
                                            >
                                                <span>Dark Mode</span>
                                            ...
                                            <button
                                                ...
                                                className={`w-full px-3 py-2 rounded-xl text-xs font-semibold flex items-center justify-between transition-colors cursor-pointer ${themeContext?.theme === 'navy' || themeContext?.theme === 'system' ? 'bg-[var(--color-primary)] text-white' : 'text-[var(--text-primary)] hover:bg-[var(--bg-surface-hover)]'}`}
                                            >
                ...
                {/* RIGHT CHAT / CONTENT COLUMN */}
                <div className="flex-1 flex flex-col min-w-0 min-h-0 bg-[var(--bg-main)] transition-colors duration-300 h-full relative">

                    {/* MINIMIZED CALL BANNER (Sits at the top of the chat area and shifts content down) */}
                    <MinimizedCallBanner />

                    <header className="md:hidden h-14 border-b border-[var(--border-color)] px-4 flex items-center justify-between bg-[var(--bg-surface)] shrink-0 z-[60] relative">
