...
import { createPortal } from 'react-dom'; // Keeping portal ONLY for sub-modals
...
const GroupMemberItem = memo(({
    ...
    if (isSelf) lastSeenEffective = null;
...
export const GroupProfileModal = memo(({ group, isOpen, onClose }) => {
    ...
    return (
        <>
            <AnimatePresence>
                {isOpen && realtimeGroup && (
                    <div
                        // EXACT FIX: Background completely removed. Just an invisible click catcher!
                        className="absolute inset-0 z-[60] flex items-center justify-center p-2 sm:p-4 select-none"
                        onClick={onClose}
                    >
                        <motion.div
                            ...
                            className="w-full h-auto max-h-[100%] max-w-md bg-[var(--bg-surface)] border border-[var(--border-color)] rounded-2xl sm:rounded-3xl p-4 sm:p-5 shadow-2xl flex flex-col relative overflow-hidden transition-colors duration-300"
                        >
                            {/* Header */}
                                ...
                                {showDeleteConfirm ? (
                                    <div className="p-4 bg-[var(--color-danger)]/10 border border-[var(--color-danger)]/20 rounded-2xl space-y-3 text-center animate-auth-card">
                                        ...
                                        <div className="flex items-center space-x-2 pt-1">
                                            <button type="button" disabled={loadingAction} onClick={() => setShowDeleteConfirm(false)} className="flex-1 py-2 bg-[var(--bg-surface-hover)] hover:opacity-90 text-[var(--text-primary)] rounded-xl text-xs font-bold border border-[var(--border-color)] transition-all cursor-pointer">
                                                Cancel
                                            </button>
                                            <button type="button" disabled={loadingAction} onClick={handleConfirmDeleteGroup} className="flex-1 py-2 bg-[var(--color-danger)] hover:opacity-90 text-white rounded-xl text-xs font-bold transition-all shadow-md cursor-pointer">
                                        ...
                                        <div className="flex flex-col items-center text-center space-y-2">
                                            ...
                                            {editing ? (
                                                <div className="w-full space-y-2 pt-2">
                                                    <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full px-3 py-1.5 bg-[var(--bg-main)] border border-[var(--border-color)] rounded-xl text-xs text-[var(--text-primary)] focus:outline-none focus:ring-1 focus:ring-[var(--color-primary)]" />
                                                    <textarea rows="2" value={description} onChange={(e) => setDescription(e.target.value)} className="w-full px-3 py-1.5 bg-[var(--bg-main)] border border-[var(--border-color)] rounded-xl text-xs text-[var(--text-primary)] resize-none focus:outline-none focus:ring-1 focus:ring-[var(--color-primary)]" />
                                                    <div className="flex justify-end space-x-2">
                                                        <button type="button" onClick={() => setEditing(false)} className="px-3 py-1 bg-[var(--bg-surface-hover)] text-xs font-semibold rounded-lg text-[var(--text-secondary)] border border-[var(--border-color)] cursor-pointer">Cancel</button>
                                                        <button type="button" disabled={loadingAction} onClick={handleSaveMetadata} className="px-3 py-1 bg-[var(--color-primary)] text-xs font-semibold rounded-lg text-white cursor-pointer">Save</button>
                                                    </div>
                                                </div>
                                            ) : (
                                                <div className="space-y-1">
                                                    <div className="flex items-center justify-center space-x-1.5">
                                                        ...
                                                        {isAdmin && (
                                                            <button type="button" onClick={() => setEditing(true)} className="text-[var(--text-secondary)] hover:text-[var(--color-primary)] p-1 cursor-pointer transition-colors" title="Edit Details">
                                                                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 210.3 3 3.5 17.768 3.696z" /></svg>
                                                            </button>
                                                        )}
                                                    </div>
            ...
            {/* Sub-Modals Escape Hatch */}
            {typeof document !== 'undefined' && createPortal(
                <>
                    {isInviteModalOpen && realtimeGroup && (
                        <InviteMembersModal
                            group={realtimeGroup}
