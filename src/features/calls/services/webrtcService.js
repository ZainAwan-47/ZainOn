...
export class WebRTCService {
    constructor() {
        ...
        this.iceCandidateQueue = []; // EXACT FIX: Queue for early candidates arriving before remoteDescription is set
    ...
    initPeerConnection() {
        ...
        this.iceCandidateQueue = []; // Reset queue on new connection
    ...
    /**
     * Helper: Flush queued ICE candidates once remote description is active
     */
    async _flushIceQueue() {
        ...
        while (this.iceCandidateQueue.length > 0) {
            ...
            try {
                ...
                console.log('[WebRTCService] Successfully flushed queued ICE candidate.');
            } catch (error) {
    ...
    async handleOfferAndCreateAnswer(offerSdp) {
        ...
        // Flush any early candidates now that remote description is set
    ...
    async addIceCandidate(candidateData) {
        if (!this.peerConnection) return;
        try {
            // Guard: If remote description is not yet set, queue the candidate instead of throwing InvalidStateError
            if (!this.peerConnection.remoteDescription || !this.peerConnection.remoteDescription.type) {
                console.log('[WebRTCService] Remote description not set yet. Queuing incoming ICE candidate.');
